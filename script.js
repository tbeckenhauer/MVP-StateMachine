...
.directive('dashboardButtonBootstrap', [function() {
    return {
      replace: true,
      template: '<button type="button" class="btn btn-default" ng-click="execCmd()">{{cmdVal}}</button>',
      scope: {
        execCmd: '&',
        cmdVal: '@',
      }
  ...
  .directive('dashboardBootstrap', [function() {
    return {
      replace: true,
      transclude: true,
      template: '<div class="btn-group" role="group" ng-transclude></div>',
    }
  }])
  .directive('messagesBootstrap', [function() {
    return {
      replace: true,
      scope: {
        msgArray: '='
